...
class ExportsReports__Prefixed__XLSXWriter
{
	...
	protected function initializeSheet($sheet_name, $col_widths=array(), $auto_filter=false, $freeze_rows=false, $freeze_columns=false )
	{
		...
		$this->sheets[$sheet_name] = (object)array(
			...
			'row_count' => 0,
			'file_writer' => new ExportsReports__Prefixed__XLSXWriter_BuffererWriter($sheet_filename),
			'columns' => array(),
			'merge_cells' => array(),
		...
		$max_cell=ExportsReports__Prefixed__XLSXWriter::xlsCell(self::EXCEL_2007_MAX_ROW, self::EXCEL_2007_MAX_COL);//XFE1048577
	...
	protected function writeCell(ExportsReports__Prefixed__XLSXWriter_BuffererWriter &$file, $row_number, $column_number, $value, $num_format_type, $cell_style_idx)
	{
	...
	protected function writeStylesXML()
	{
		...
		$file = new ExportsReports__Prefixed__XLSXWriter_BuffererWriter($temporary_filename);
...
class ExportsReports__Prefixed__XLSXWriter_BuffererWriter
{
	protected $fd=null;
	protected $buffer='';
	...
	public function __construct($filename, $fd_fopen_flags='w', $check_utf8=false)
	{
		...
		if ($this->fd===false) {
			ExportsReports__Prefixed__XLSXWriter::log("Unable to open $filename for writing.");
		}
	}
	...
	protected function purge()
	{
		if ($this->fd) {
			if ($this->check_utf8 && !self::isValidUTF8($this->buffer)) {
				ExportsReports__Prefixed__XLSXWriter::log("Error, invalid UTF8 encoding detected.");
