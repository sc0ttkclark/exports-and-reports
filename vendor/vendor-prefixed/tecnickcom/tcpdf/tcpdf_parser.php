...
/**
 * @class ExportsReports__Prefixed__TCPDF_PARSER
 * This is a PHP class for parsing PDF documents.<br>
 * @package com.tecnick.tcpdf
...
// -----------------------------------------------------------------------------
	...
	protected function decodeStream($sdic, $stream) {
		...
		foreach ($filters as $filter) {
			if (in_array($filter, ExportsReports__Prefixed__TCPDF_FILTERS::getAvailableFilters())) {
				try {
					$stream = ExportsReports__Prefixed__TCPDF_FILTERS::decodeFilter($filter, $stream);
				} catch (Exception $e) {
					$emsg = $e->getMessage();
	...
	public function Error($msg) {
		if ($this->cfg['die_for_errors']) {
			die('<strong>ExportsReports__Prefixed__TCPDF_PARSER ERROR: </strong>'.$msg);
		} else {
			throw new Exception('ExportsReports__Prefixed__TCPDF_PARSER ERROR: '.$msg);
		}
	}

} // END OF ExportsReports__Prefixed__TCPDF_PARSER CLASS
