...
//
// Description :
//   Static image methods used by the ExportsReports__Prefixed__TCPDF class.
//
//============================================================+

/**
 * @file
 * This is a PHP class that contains static image methods for the ExportsReports__Prefixed__TCPDF class.<br>
...
/**
 * @class ExportsReports__Prefixed__TCPDF_IMAGES
 * Static image methods used by the ExportsReports__Prefixed__TCPDF class.
 * @package com.tecnick.tcpdf
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	public static function _parsejpeg($file) {
		...
		if (!@ExportsReports__Prefixed__TCPDF_STATIC::file_exists($file)) {
			return false;
		}
		$a = getimagesize($file);
		if (empty($a)) {
		...
		while (($pos = strpos($data, "ICC_PROFILE\0", $offset)) !== false) {
			...
			$length = (ExportsReports__Prefixed__TCPDF_STATIC::_getUSHORT($data, ($pos - 2)) - 16);
	...
	public static function _parsepng($file) {
		...
		$w = ExportsReports__Prefixed__TCPDF_STATIC::_freadint($f);
		$h = ExportsReports__Prefixed__TCPDF_STATIC::_freadint($f);
		$bpc = ord(fread($f, 1));
		$ct = ord(fread($f, 1));
		if ($ct == 0) {
			$colspace = 'DeviceGray';
		...
		$data = '';
		$icc = false;
		$n = ExportsReports__Prefixed__TCPDF_STATIC::_freadint($f);
		do {
			$type = fread($f, 4);
			if ($type == 'PLTE') {
				// read palette
				$pal = ExportsReports__Prefixed__TCPDF_STATIC::rfread($f, $n);
				fread($f, 4);
			} elseif ($type == 'tRNS') {
				// read transparency info
				$t = ExportsReports__Prefixed__TCPDF_STATIC::rfread($f, $n);
				if ($ct == 0) { // DeviceGray
					$trns = array(ord($t[1]));
				...
				$data .= ExportsReports__Prefixed__TCPDF_STATIC::rfread($f, $n);
				fread($f, 4);
			} elseif ($type == 'iCCP') {
				// skip profile name
				$len = 0;
				...
				$icc = ExportsReports__Prefixed__TCPDF_STATIC::rfread($f, ($n - $len - 2));
				// decompress profile
				...
				ExportsReports__Prefixed__TCPDF_STATIC::rfread($f, $n + 4);
			}
			$n = ExportsReports__Prefixed__TCPDF_STATIC::_freadint($f);
		} while ($n);
...
} // END OF ExportsReports__Prefixed__TCPDF_IMAGES CLASS
