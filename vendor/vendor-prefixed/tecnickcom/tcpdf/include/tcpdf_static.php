...
//
// Description :
//   Static methods used by the ExportsReports__Prefixed__TCPDF class.
//
//============================================================+

/**
 * @file
 * This is a PHP class that contains static methods for the ExportsReports__Prefixed__TCPDF class.<br>
...
/**
 * @class ExportsReports__Prefixed__TCPDF_STATIC
 * Static methods used by the ExportsReports__Prefixed__TCPDF class.
 * @package com.tecnick.tcpdf
...
class ExportsReports__Prefixed__TCPDF_STATIC {
	...
	/**
	 * Current ExportsReports__Prefixed__TCPDF version.
	 * @private static
	 */
	...
	/**
	 * Return the current ExportsReports__Prefixed__TCPDF version.
	 * @return string ExportsReports__Prefixed__TCPDF version string
	...
	/**
	 * Return the current ExportsReports__Prefixed__TCPDF producer.
	 * @return string ExportsReports__Prefixed__TCPDF producer string
	 ...
	 * @param string $file_id ExportsReports__Prefixed__TCPDF file_id.
	 * @return string filename.
	 * @since 4.5.000 (2008-12-31)
	 * @public static
	 */
	public static function getObjFilename($type='tmp', $file_id='') {
		return tempnam(K_PATH_CACHE, '__tcpdf_'.$file_id.'_'.$type.'_'.md5(ExportsReports__Prefixed__TCPDF_STATIC::getRandomSeed()).'_');
	}
	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function getAnnotOptFromJSProp($prop, &$spot_colors, $rtl=false) {
		...
		if (isset($prop['fillColor'])) {
			if (is_array($prop['fillColor'])) {
				...
				$opt['mk']['bg'] = ExportsReports__Prefixed__TCPDF_COLORS::convertHTMLColorToDec($prop['fillColor'], $spot_colors);
			}
		}
		...
		if (isset($prop['strokeColor'])) {
			if (is_array($prop['strokeColor'])) {
				...
				$opt['mk']['bc'] = ExportsReports__Prefixed__TCPDF_COLORS::convertHTMLColorToDec($prop['strokeColor'], $spot_colors);
			}
		}
	...
	public static function fixHTMLCode($html, $default_css, $tagvs, $tidy_options, &$tagvspaces) {
		...
		if (ExportsReports__Prefixed__TCPDF_STATIC::empty_string($tidy_options)) {
			$tidy_options = array (
				'clean' => 1,
				'drop-empty-paras' => 0,
		...
		if (!ExportsReports__Prefixed__TCPDF_STATIC::empty_string($tagvs)) {
	...
	public static function getHyphenPatternsFromTEX($file) {
		...
		foreach($patterns_array as $val) {
			if (!ExportsReports__Prefixed__TCPDF_STATIC::empty_string($val)) {
				$val = trim($val);
				$val = str_replace('\'', '\\\'', $val);
...
} // END OF ExportsReports__Prefixed__TCPDF_STATIC CLASS
