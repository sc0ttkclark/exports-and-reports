...
function exports_reports_view( $group_id = false ) {
	...
	$has_full_access = ExportReports::current_user_can_any( 'exports_reports_full_access' );
			...
			foreach ( $groups as $group ) {
				...
				if ( 0 < @count( $reports ) ) {
					foreach ( $reports as $report ) {
						if ( $has_full_access || ExportReports::current_user_can_any( 'exports_reports_view' ) || ExportReports::current_user_can_any( 'exports_reports_view_group_' . $group->id ) || ExportReports::current_user_can_any( 'exports_reports_view_report_' . $report->id ) ) {
							$group_id = $group->id;
	...
	foreach ( $reports as $report ) {
		if ( $has_full_access || ExportReports::current_user_can_any( 'exports_reports_view' ) || ExportReports::current_user_can_any( 'exports_reports_view_group_' . $group_id ) || ExportReports::current_user_can_any( 'exports_reports_view_report_' . $report->id ) ) {
